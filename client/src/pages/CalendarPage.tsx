...
export default function CalendarPage() {
  ...
  const { data: events = [], isLoading } = useQuery({
    ...
    queryFn: () => tripApi.calendar(view.getMonth() + 1, view.getFullYear()),
  });
  const days = Array.from(
    { length: new Date(view.getFullYear(), view.getMonth() + 1, 0).getDate() },
    (_, index) => index + 1
  );
  const lead =
  ...
  const eventOn = (day: number) =>
    events.some(event => {
      ...
      return (
        current >= new Date(event.startDate).setHours(0, 0, 0, 0) &&
        current <= new Date(event.endDate).setHours(23, 59, 59, 999)
      );
    });
  const next = events[0];
    ...
    <AppShell>
      <section className="mx-auto max-w-[1120px] px-5 py-12 lg:px-8">
        <PageIntro
          ...
          title={
            <>
              Travel calendar<span className="text-[var(--gold)]">.</span>
            </>
          }
          description="Every saved travel day and good little promise in one quiet view."
        />
        <div className="mt-10 grid gap-7 lg:grid-cols-[1.25fr_.75fr]">
          <article className="rounded-[28px] border border-[var(--line)] bg-white p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => move(-1)}
                  className="rounded-full border border-[var(--line)] p-2 text-[var(--navy)]"
                >
                  <ArrowLeft size={16} />
                </button>
                <button
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {days.map(day => (
                  <div
                    key={day}
                    className={`relative flex aspect-square items-center justify-center rounded-xl text-sm font-bold ${eventOn(day) ? "bg-[var(--navy)] text-white shadow-md" : "text-[var(--ink-muted)] hover:bg-[var(--sand)]"}`}
                  >
                    {day}
                    {eventOn(day) ? (
          ...
          <aside className="rounded-[28px] bg-[var(--sand)] p-7">
            ...
            {isLoading ? (
              ...
              <div>
                ...
                <div className="my-7 h-px bg-[var(--sand-dark)]" />
                <div className="space-y-4 text-sm font-semibold text-[var(--ink-muted)]">
              ...
              <p className="mt-3 text-sm leading-6 text-[var(--ink-muted)]">
                No saved trip touches this month yet. Create a route and its
