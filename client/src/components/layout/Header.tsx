    ...
    <header className="sticky top-0 z-50 border-b border-[var(--line)] bg-[color:var(--canvas)]/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[74px] max-w-[1240px] items-center justify-between px-5 lg:px-8">
        <Link href="/" className="flex items-center gap-3" aria-label="World Trotter home">
          <BrandMark size={42} />
          <span className="font-serif text-[26px] font-bold tracking-[-0.05em] text-[var(--navy)]">World <span className="text-[var(--gold)]">Trotter</span></span>
        </Link>
        <nav className="hidden items-center gap-8 lg:flex" aria-label="Primary navigation">
          {navigation.map(([label, href]) => {
            const active = location === href || (href === "/trips" && location.startsWith("/trips/"));
            return <Link key={href} href={href} className={`relative py-7 text-[15.5px] font-bold tracking-[0.01em] ${active ? "text-[var(--navy)]" : "text-[var(--ink-muted)] hover:text-[var(--navy)]"}`}>
      ...
      <div className="route-ribbon hidden items-center gap-3 overflow-hidden border-t border-[var(--line)] px-8 py-2 text-[11.5px] font-bold uppercase tracking-[0.14em] text-[var(--ink-muted)] lg:flex">
